...
async function generateQWeatherJwt(config) {
  ...
  const signature = await signEd25519Message(config.privateKeyBase64, utf8Bytes(signingInput));

  return `${signingInput}.${base64UrlEncodeBytes(new Uint8Array(signature))}`;
}

async function signEd25519Message(privateKeyBase64, messageBytes) {
  if (globalThis.crypto?.subtle) {
    const privateKey = await importEd25519PrivateKey(privateKeyBase64);
    return crypto.subtle.sign("Ed25519", privateKey, messageBytes);
  }

  const seed = extractEd25519Seed(privateKeyBase64);
...
function extractEd25519Seed(value) {
  const bytes = extractPkcs8Bytes(value);
  ...
  throw new Error("Failed to extract Ed25519 seed from private key.");
}
